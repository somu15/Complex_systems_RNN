#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Sun Aug 23 16:23:55 2020

@author: som
"""

# Imports

import os
import pathlib
import matplotlib.pyplot as plt
import numpy as np
import pandas as pd
import seaborn as sns
os.chdir('/Users/som/Dropbox/Complex_systems_RNN/DL_tutorial')
from Kij import Kij

import tensorflow as tf
from tensorflow import keras
from tensorflow.keras import layers
from tensorflow.keras import regularizers
print(tf.__version__)
import tensorflow_docs as tfdocs
import tensorflow_docs.plots
import tensorflow_docs.modeling

# Import dataset

dataset_path = '/Users/som/Dropbox/Complex_systems_RNN/Data/DL_test_data_Eq_Hazus.csv'
# column_names = ['Time','P1','P2','IM','Rec']

# dataset = pd.read_csv(dataset_path, names=column_names,
#                       na_values = "?", comment='\t',
#                       sep=" ", skipinitialspace=True)

dataset = pd.read_csv(dataset_path)

# dataset["IM"] = np.log(dataset["IM"])
# dataset.pop("P1")
# dataset.pop("P2")

Rec = dataset['Rec']
r1 = Rec == 0.0
r2 = Rec == 1.0
Rec[r1] = 0.000001
Rec[r2] = 0.999999
Rec = -np.log(1/Rec - 1)
dataset['Rec'] = Rec

Rec = dataset['P1']
r1 = Rec == 0.0
r2 = Rec == 1.0
Rec[r1] = 0.000001
Rec[r2] = 0.999999
Rec = -np.log(1/Rec - 1)
dataset['P1'] = Rec

Rec = dataset['P2']
r1 = Rec == 0.0
r2 = Rec == 1.0
Rec[r1] = 0.000001
Rec[r2] = 0.999999
Rec = -np.log(1/Rec - 1)
dataset['P2'] = Rec

dataset.tail()

# Split the data into train and test

train_dataset = dataset.sample(frac=0.8,random_state=100)
test_dataset = dataset.drop(train_dataset.index)

# Inspect the data

sns.pairplot(train_dataset[["Rec", "IM", "P1", "P2", "Time"]], diag_kind="kde")
# sns.pairplot(train_dataset[["Rec", "IM"]], diag_kind="kde")

train_stats = train_dataset.describe()
train_stats.pop("Rec")
train_stats = train_stats.transpose()
train_stats

# Split features from labels

train_labels = train_dataset.pop('Rec')
test_labels = test_dataset.pop('Rec')

# Normalize the data

def norm(x):
  return (x - train_stats['mean']) / train_stats['std']
normed_train_data = norm(train_dataset)
normed_test_data = norm(test_dataset)

# Build the model ,kernel_regularizer='l2'

def build_model():
  model = keras.Sequential([
    layers.Dense(8, activation='sigmoid', input_shape=[len(train_dataset.keys())]),
    layers.Dense(4, activation='sigmoid'),
    layers.Dense(1)
  ])

  optimizer = tf.keras.optimizers.RMSprop(0.001)

  model.compile(loss='mse',
                optimizer=optimizer,
                metrics=['mae', 'mse'])
  return model

model = build_model()

# Inspect the model

model.summary()

example_batch = normed_train_data[:10]
example_result = model.predict(example_batch)
example_result

# Train the model

EPOCHS = 5000

history = model.fit(
  normed_train_data, train_labels,
  epochs=EPOCHS, validation_split = 0.2, verbose=0,
  callbacks=[tfdocs.modeling.EpochDots()])

hist = pd.DataFrame(history.history)
hist['epoch'] = history.epoch
hist.tail()

plotter = tfdocs.plots.HistoryPlotter(smoothing_std=2)
plotter.plot({'Basic': history}, metric = "mae")
plt.ylim([0, 25])
plt.ylabel('MAE [Rec]')

plotter.plot({'Basic': history}, metric = "mse")
plt.ylim([0, 25])
plt.ylabel('MSE [Rec]')

# Callback

# model = build_model()

# The patience parameter is the amount of epochs to check for improvement
# early_stop = keras.callbacks.EarlyStopping(monitor='val_loss', patience=10)

# early_history = model.fit(normed_train_data, train_labels, 
#                     epochs=EPOCHS, validation_split = 0.2, verbose=0, 
#                     callbacks=[early_stop, tfdocs.modeling.EpochDots()])
# plotter.plot({'Early Stopping': early_history}, metric = "mae")
# plt.ylim([0, 10])
# plt.ylabel('MAE [Rec]')
# loss, mae, mse = model.evaluate(normed_test_data, test_labels, verbose=2)
# print("Testing set Mean Abs Error: {:5.2f} Rec".format(mae))

def sigm(x):
    return (1/(1+np.exp(-x)))

# Make predictions

test_predictions = model.predict(normed_test_data).flatten()

a = plt.axes(aspect='equal')
plt.scatter(sigm(test_labels), sigm(test_predictions))
plt.xlabel('True Values [Rec]')
plt.ylabel('Predictions [Rec]')
lims = [0, 1]
plt.xlim(lims)
plt.ylim(lims)
_ = plt.plot(lims, lims)

error = sigm(test_predictions) - sigm(test_labels)
plt.hist(error, bins = 25)
plt.xlabel("Prediction Error [Rec]")
_ = plt.ylabel("Count")

## Verify model

# dataset_path1 = '/Users/som/Dropbox/Complex_systems_RNN/Data/DL_verify_EQ_1_3.csv'

# data1 = pd.read_csv(dataset_path1)

# Rec = data1['Rec']
# r1 = Rec == 0.0
# r2 = Rec == 1.0
# Rec[r1] = 0.001
# Rec[r2] = 0.999
# Rec = -np.log(1/Rec - 1)
# data1['Rec'] = Rec

# Rec = data1['P1']
# r1 = Rec == 0.0
# r2 = Rec == 1.0
# Rec[r1] = 0.001
# Rec[r2] = 0.999
# Rec = -np.log(1/Rec - 1)
# data1['P1'] = Rec

# Rec = data1['P2']
# r1 = Rec == 0.0
# r2 = Rec == 1.0
# Rec[r1] = 0.001
# Rec[r2] = 0.999
# Rec = -np.log(1/Rec - 1)
# data1['P2'] = Rec

# data1.tail()

# data1_labels = data1.pop('Rec')

# normed_data1 = norm(data1)

# data1_pred = model.predict(normed_data1).flatten()

# a = plt.axes(aspect='equal')
# plt.scatter(sigm(data1_labels), sigm(data1_pred))
# plt.xlabel('True Values [Rec]')
# plt.ylabel('Predictions [Rec]')
# lims = [0, 1]
# plt.xlim(lims)
# plt.ylim(lims)
# _ = plt.plot(lims, lims)

# plt.plot(data1['Time'],sigm(data1_labels),label='Exact')
# plt.plot(data1['Time'],sigm(data1_pred),label='Prediction')
# plt.xlabel('Time [days]')
# plt.ylabel('Functionality')
# plt.xlim([0, 2000])
# plt.ylim([0, 1])
# plt.legend()

## Verify model for multiple recovery curves

dataset_path1 = '/Users/som/Dropbox/Complex_systems_RNN/Data/DL_verify_EQ_0_7.csv'
data1 = pd.read_csv(dataset_path1)
# data1["IM"] = np.log(data1["IM"])
# data1.pop("IM")
# data1.pop("P1")
# data1.pop("P2")
Rec = data1['Rec']
r1 = Rec == 0.0
r2 = Rec == 1.0
Rec[r1] = 0.000001
Rec[r2] = 0.999999
Rec = -np.log(1/Rec - 1)
data1['Rec'] = Rec
Rec = data1['P1']
r1 = Rec == 0.0
r2 = Rec == 1.0
Rec[r1] = 0.000001
Rec[r2] = 0.999999
Rec = -np.log(1/Rec - 1)
data1['P1'] = Rec
Rec = data1['P2']
r1 = Rec == 0.0
r2 = Rec == 1.0
Rec[r1] = 0.000001
Rec[r2] = 0.999999
Rec = -np.log(1/Rec - 1)
data1['P2'] = Rec
data1_labels = data1.pop('Rec')
normed_data1 = norm(data1)
data1_pred = model.predict(normed_data1).flatten()

dataset_path2 = '/Users/som/Dropbox/Complex_systems_RNN/Data/DL_verify_EQ_0_9.csv'
data2 = pd.read_csv(dataset_path2)
# data2["IM"] = np.log(data2["IM"])
# data2.pop("IM")
# data2.pop("P1")
# data2.pop("P2")
Rec = data2['Rec']
r1 = Rec == 0.0
r2 = Rec == 1.0
Rec[r1] = 0.000001
Rec[r2] = 0.999999
Rec = -np.log(1/Rec - 1)
data2['Rec'] = Rec
Rec = data2['P1']
r1 = Rec == 0.0
r2 = Rec == 1.0
Rec[r1] = 0.000001
Rec[r2] = 0.999999
Rec = -np.log(1/Rec - 1)
data2['P1'] = Rec
Rec = data2['P2']
r1 = Rec == 0.0
r2 = Rec == 1.0
Rec[r1] = 0.000001
Rec[r2] = 0.999999
Rec = -np.log(1/Rec - 1)
data2['P2'] = Rec
data2_labels = data2.pop('Rec')
normed_data2 = norm(data2)
data2_pred = model.predict(normed_data2).flatten()

dataset_path3 = '/Users/som/Dropbox/Complex_systems_RNN/Data/DL_verify_EQ_1_1.csv'
data3 = pd.read_csv(dataset_path3)
# data3["IM"] = np.log(data3["IM"])
# data3.pop("IM")
# data3.pop("P1")
# data3.pop("P2")
Rec = data3['Rec']
r1 = Rec == 0.0
r2 = Rec == 1.0
Rec[r1] = 0.000001
Rec[r2] = 0.999999
Rec = -np.log(1/Rec - 1)
data3['Rec'] = Rec
Rec = data3['P1']
r1 = Rec == 0.0
r2 = Rec == 1.0
Rec[r1] = 0.000001
Rec[r2] = 0.999999
Rec = -np.log(1/Rec - 1)
data3['P1'] = Rec
Rec = data3['P2']
r1 = Rec == 0.0
r2 = Rec == 1.0
Rec[r1] = 0.000001
Rec[r2] = 0.999999
Rec = -np.log(1/Rec - 1)
data3['P2'] = Rec
data3_labels = data3.pop('Rec')
normed_data3 = norm(data3)
data3_pred = model.predict(normed_data3).flatten()

dataset_path4 = '/Users/som/Dropbox/Complex_systems_RNN/Data/DL_verify_EQ_1_3.csv'
data4 = pd.read_csv(dataset_path4)
# data4["IM"] = np.log(data4["IM"])
# data4.pop("IM")
# data4.pop("P1")
# data4.pop("P2")
Rec = data4['Rec']
r1 = Rec == 0.0
r2 = Rec == 1.0
Rec[r1] = 0.000001
Rec[r2] = 0.999999
Rec = -np.log(1/Rec - 1)
data4['Rec'] = Rec
Rec = data4['P1']
r1 = Rec == 0.0
r2 = Rec == 1.0
Rec[r1] = 0.000001
Rec[r2] = 0.999999
Rec = -np.log(1/Rec - 1)
data4['P1'] = Rec
Rec = data4['P2']
r1 = Rec == 0.0
r2 = Rec == 1.0
Rec[r1] = 0.000001
Rec[r2] = 0.999999
Rec = -np.log(1/Rec - 1)
data4['P2'] = Rec
data4_labels = data4.pop('Rec')
normed_data4 = norm(data4)
data4_pred = model.predict(normed_data4).flatten()

dataset_path5 = '/Users/som/Dropbox/Complex_systems_RNN/Data/DL_verify_EQ_1_5.csv'
data5 = pd.read_csv(dataset_path5)
# data5["IM"] = np.log(data5["IM"])
# data5.pop("IM")
# data5.pop("P1")
# data5.pop("P2")
Rec = data5['Rec']
r1 = Rec == 0.0
r2 = Rec == 1.0
Rec[r1] = 0.000001
Rec[r2] = 0.999999
Rec = -np.log(1/Rec - 1)
data5['Rec'] = Rec
Rec = data5['P1']
r1 = Rec == 0.0
r2 = Rec == 1.0
Rec[r1] = 0.000001
Rec[r2] = 0.999999
Rec = -np.log(1/Rec - 1)
data5['P1'] = Rec
Rec = data5['P2']
r1 = Rec == 0.0
r2 = Rec == 1.0
Rec[r1] = 0.000001
Rec[r2] = 0.999999
Rec = -np.log(1/Rec - 1)
data5['P2'] = Rec
data5_labels = data5.pop('Rec')
normed_data5 = norm(data5)
data5_pred = model.predict(normed_data5).flatten()

dataset_path6 = '/Users/som/Dropbox/Complex_systems_RNN/Data/DL_verify_EQ_1_7.csv'
data6 = pd.read_csv(dataset_path6)
# data6["IM"] = np.log(data6["IM"])
# data6.pop("IM")
# data6.pop("P1")
# data6.pop("P2")
Rec = data6['Rec']
r1 = Rec == 0.0
r2 = Rec == 1.0
Rec[r1] = 0.000001
Rec[r2] = 0.999999
Rec = -np.log(1/Rec - 1)
data6['Rec'] = Rec
Rec = data6['P1']
r1 = Rec == 0.0
r2 = Rec == 1.0
Rec[r1] = 0.000001
Rec[r2] = 0.999999
Rec = -np.log(1/Rec - 1)
data6['P1'] = Rec
Rec = data6['P2']
r1 = Rec == 0.0
r2 = Rec == 1.0
Rec[r1] = 0.000001
Rec[r2] = 0.999999
Rec = -np.log(1/Rec - 1)
data6['P2'] = Rec
data6_labels = data6.pop('Rec')
normed_data6 = norm(data6)
data6_pred = model.predict(normed_data6).flatten()

a = plt.axes(aspect='equal')
plt.scatter(sigm(data1_labels), sigm(data1_pred))
plt.scatter(sigm(data2_labels), sigm(data2_pred))
plt.scatter(sigm(data3_labels), sigm(data3_pred))
plt.scatter(sigm(data4_labels), sigm(data4_pred))
plt.scatter(sigm(data5_labels), sigm(data5_pred))
plt.scatter(sigm(data6_labels), sigm(data6_pred))
plt.xlabel('True Values [Rec]')
plt.ylabel('Predictions [Rec]')
lims = [0, 1]
plt.xlim(lims)
plt.ylim(lims)
_ = plt.plot(lims, lims)

plt.plot(data1['Time'],sigm(data1_labels))
plt.plot(data1['Time'],sigm(data1_pred))
plt.plot(data2['Time'],sigm(data2_labels))
plt.plot(data2['Time'],sigm(data2_pred))
plt.plot(data3['Time'],sigm(data3_labels))
plt.plot(data3['Time'],sigm(data3_pred))
plt.plot(data4['Time'],sigm(data4_labels))
plt.plot(data4['Time'],sigm(data4_pred))
plt.plot(data5['Time'],sigm(data5_labels))
plt.plot(data5['Time'],sigm(data5_pred))
plt.plot(data6['Time'],sigm(data6_labels))
plt.plot(data6['Time'],sigm(data6_pred))
plt.xlabel('Time [days]')
plt.ylabel('Functionality')
plt.xlim([0, 800])
plt.ylim([0, 1])
# plt.legend()

## Predict the disfunctionality hazard curve

IMe = (np.arange(0.01,2.01,0.01))
IMe.reshape(len(IMe),1)
Tim = (np.arange(4,1000,1))
Tim.reshape(len(Tim),1)

Res = np.zeros((len(IMe),len(Tim)))

for ii in np.arange(0,len(IMe),1):
    dat = pd.DataFrame(Tim,columns=['Time'])
    Dis_tim = Kij(IM=IMe[ii],Haz='Eq',time=dat['Time'])
    A1,A2 = Dis_tim.Time()
    dat['P1'] = A1.reshape(len(dat['Time']),1)
    dat['P2'] = A2.reshape(len(dat['Time']),1)
    dat['IM'] = IMe[ii]*np.ones((len(dat['Time']),1))
    # dat['IM'] = np.log(dat['IM'])
    Rec = dat['P1']
    r1 = Rec == 0.0
    r2 = Rec == 1.0
    Rec[r1] = 0.000001
    Rec[r2] = 0.999999
    Rec = -np.log(1/Rec - 1)
    dat['P1'] = Rec
    Rec = dat['P2']
    r1 = Rec == 0.0
    r2 = Rec == 1.0
    Rec[r1] = 0.000001
    Rec[r2] = 0.999999
    Rec = -np.log(1/Rec - 1)
    dat['P2'] = Rec
    normed_dat = norm(dat)
    dat_pred = model.predict(normed_dat).flatten()
    Res[ii,:] = 1-sigm(dat_pred)
    # plt.plot(dat['Time'],sigm(dat_pred))
    # plt.xlim([0, 800])
    # plt.ylim([0, 1])
    
dat_haz = '/Users/som/Dropbox/Complex_systems_RNN/Data/Eq_Haz.csv'
dat_haz = pd.read_csv(dat_haz)

Disf = np.zeros((len(Tim),1))

for ii in np.arange(0,len(Tim),1):
    # Disf[ii] = np.sum(dat_haz['Haz_diff']*Res[:,ii])*0.01
    Disf[ii] = np.trapz(dat_haz['Haz_diff']*Res[:,ii],IMe)

Disf_comp = pd.read_csv('/Users/som/Dropbox/Complex_systems_RNN/Data/Eq_Disf_Exact.csv')

plt.loglog(Tim,Disf,Disf_comp['Time'],Disf_comp['Disf'])